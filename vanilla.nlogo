globals 
[
  city-patches                      ;; agentset of grey patches - represents home locations
...
to setup-patches
  ...
  ;; create the 'city'
  set city-patches patches with [pycor > 0 and pycor <= 39]
  ask city-patches [ set pcolor 38 ]

  ;; create the 'wetland'
  set wetland-patches patches with [pycor >= 41 and pycor <= 50]
  ...
  ;; create the 'boundary'
  set boundary-patches patches with [pycor = 40]
...
to setup-people
  ...
  [
    ...
    set color red                         ;;  
    set experience 0.1 * (random 100)            ;;  initialise the tourist's history of wetland visits
...
to move-tourists
    ...
    [
      ...
      let newycor ycor + (random 3) - 1                         ;;  move tourist randomly in y
      
      if (visiting-wetland? = false)
      [
        if ((random 100) <= probability-of-recreation)          ;;  probability-of-recreation is the chance of making a recreation choice (either beach or wetland) each time-step                            
          [
          ifelse ((random 100) <= experience)                   ;;  if experience is high then tourist is more likely to make the wetland choice when recreating
            [
      ...
      if (newycor = 0)                                          ;;  send tourist back to city
      [
        set newycor 20                                         
      ]
      if (newycor = 40 and visiting-wetland? = false)           ;;  send tourist back to city
      [
        set newycor 20
      ]
      if (newycor = 40 and visiting-wetland? = true)            ;;  tourist exits the wetland
      [
        set visiting-wetland? false
        set time-since-wetland-visit 0                          ;;  sets counter for wetland visit history to 0
        set newycor 20
      ]
      if (newycor = 51)                                         ;;  tourist exits the wetland 
      [
        set visiting-wetland? false
        set time-since-wetland-visit 0                          ;;  sets counter for wetland visit history to 0
        set newycor 20
      ...
      [
        if (count tourists-here > 1)
        [
          ifelse (max [gossip-weight] of tourists-here = 1) [set experience [experience] of one-of tourists-here with [gossip-weight = 1]] [set experience (1 - gossip-weight) * experience + ((gossip-weight) * (mean [experience] of other tourists-here))]   ;; calculate experience as weight average of tourists on same patch
        ]
...
to move-ecologists
  ...
  [
    ...
    if newycor = 40 [set newycor 50]
    if newycor = 51 [set newycor 41]
    setxy xcor newycor
