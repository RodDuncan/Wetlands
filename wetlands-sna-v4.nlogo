...
to setup-patches
  ...
  ask patches 
  [ 
    set infrastructure sqrt((random max-resources-patch) / max-resources-patch) * 10  ;;  initialise patch resource levels
    set ecology sqrt((random max-resources-patch) / max-resources-patch) * 10
  ]
...
to setup-networks
  ...
  ask n-of number-long-links tourists with [any? my-links != false] 
  [
    show self
    show sort my-links
    if (any? my-links != false) [ask one-of my-links [die]]
...
to layout
  ...
  [
    move-to one-of patches with [(distance myself < 3) and (village-here = [village] of myself)]
  ]
...
to move-tourists
    ...
    [
      ...
      [
        ...
        set time-spent-in-wetland time-spent-in-wetland + 1
      ]
      [
        if ((random 1000) <= probability-of-recreation)          ;;  probability-of-recreation is the chance of making a recreation choice (either beach or wetland) each time-step                            
          [
...
to move-builders
  ...
  [
    ...
    [
      set resources resources - 1
      set infrastructure infrastructure + 1
      set pcolor 69 -  int ((infrastructure / max-resources-patch) * 5)
     ]
  ]
...
to go
  ...
  set average-time-spent-in-wetland mean [time-spent-in-wetland] of tourists
  tick
end
@#$#@#$#@
GRAPHICS-WINDOW
...
578
NIL
mean [experience] of tourists with [visiting-wetland? = true]
17
1
11
...
@#$#@#$#@
<experiments>
  <experiment name="Run 3" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="10000"/>
    <metric>average-infrastructure</metric>
    <steppedValueSet variable="number-long-links" first="0" step="2" last="10"/>
    <enumeratedValueSet variable="tourist-payment">
      <value value="40"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
